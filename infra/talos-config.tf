...
ephemeral "talos_machine_configuration" "control_plane_nodes" {
  ...
  config_patches = flatten([
    ...
    templatefile("${path.module}/files/service-account-issuer.yaml.tmpl", {
      issuer = var.cluster_service_account_issuer,
    }),
    ...
    templatefile("${path.module}/files/authn-config.yaml.tmpl", {
